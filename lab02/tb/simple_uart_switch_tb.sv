...
//------------------------------------------------------------------------------
    ...
    covergroup address_cov;
        ...
        coverpoint addr_cov {
            // Program all addresses 0..255 [A1, A2]
            bins all_addrs_prog[]    = {[0:255]} iff (prog_cov == 1);
            // Use all addresses 0..255 in normal mode [A1, A2]
    ...
    bit [2:0] invalid_type_cov; // [0]=start_err, [1]=stop_err, [2]=parity_err
    covergroup routing_data_cov;
        option.name = "cg_routing_and_data";
        
        // Chceck which port is programmed for routing [A1, A2]
        cp_programmed: coverpoint port_cov {
            // programmed to sout0 (0x00) [A1]
            bins sout0_programmed = {8'h00} iff (prog_cov == 1);
            // programmed to sout1 (0x01) [A2]
            bins sout1_programmed = {8'h01} iff (prog_cov == 1);
        }

        // Valid and invalid packet coverage [A4, A5, A6, A7]
        cp_valid_to_port: coverpoint packet_valid_cov {
            bins valid_sout0   = {1'b1} iff (port_cov == 8'h00); // valid packet routed to sout0 [A4]
            bins valid_sout1   = {1'b1} iff (port_cov == 8'h01); // valid packet routed to sout1 [A5]
            bins invalid_sout0 = {1'b0} iff (port_cov == 8'h00); // invalid packet routed to sout0 [A6]
            bins invalid_sout1 = {1'b0} iff (port_cov == 8'h01); // invalid packet routed to sout1 [A7]
        }

        // Invalid packet type coverage [A8, A9, A10]
        cp_invalid_types: coverpoint invalid_type_cov {
            // invalid_type_cov = {start_err, stop_err, parity_err}
            bins no_error        = {3'b000};
            bins start_error     = {3'b100};
            bins stop_error      = {3'b010};
            bins parity_error    = {3'b001};
            bins start_stop      = {3'b110};
            bins start_parity    = {3'b101};
            bins stop_parity     = {3'b011};
            bins all_errors      = {3'b111};
        }

        // Data value coverage [A1, A2]
    ...
    final begin
        ...
        $display("Routing and data coverage: %.2f%%", rout_data_cg.get_coverage());
...
//------------------------------------------------------------------------------
    ...
    task automatic create_and_send_transaction(
        ...
        gen2drv_mb.put(tr);
        gen2cov_mb.put(tr); // also to coverage
